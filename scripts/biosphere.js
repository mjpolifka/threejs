...
const hemisphere = new THREE.HemisphereLight(0xd8eeff, 0x335640, 0.75);
...
for (let i = 0; i < pos.count; i += 1) {
  ...
  const z = pos.getZ(i);
  const hill =
    Math.sin(x * 0.08) * Math.cos(z * 0.07) * 2.7 +
    Math.sin((x + z) * 0.05) * 1.4 +
    Math.cos((x - z) * 0.06) * 1.2;
  pos.setY(i, hill);
